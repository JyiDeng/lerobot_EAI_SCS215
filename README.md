...
我们修改的主要文件，分别处理机械臂配置、飞特硬件型号配置、通用串口读写、SCS215 配置和硬件数值映射。

```
机械臂功能层
SOFollower（so_follower.py）管机械臂：
连接、校准、读取状态、发送目标位置
        │ 使用
        ▼
飞特硬件适配层
FeetechMotorsBus（feetech.py）管飞特舵机：
飞特协议、型号检查、飞特专用配置
        │ 继承
        ▼
通用串口总线层
SerialMotorsBus（motors_bus.py）提供通用串口读写：
单个读取、单个写入、同步写入、范围记录

tables.py 给读写操作提供硬件数值：
向 FeetechMotorsBus 提供型号、寄存器地址、
分辨率、波特率和编码规则
```

| 文件 | 代码实际作用与限制 | 针对 SCS215 的修改 | 
| --- | --- | --- |
| [`so_follower.py`](./src/lerobot/robots/so_follower/so_follower.py) |这个文件决定**机械臂配置**，包括关节数量、舵机型号、连接、校准和运动流程。 |六个关节的 **电机型号** 从 `sts3215` 改为 `scs215`，**总线协议** 改为 `1`，机身关节使用 **归一化位置**。当前位置改为**逐个读取**，每次读取使用 `num_read_retries` 设定的 **读取重试次数**。校准时记录六个关节的**实际安全活动范围**，腕部旋转关节不再固定为 `0` 至 `1023`。SCS215 同时跳过工作模式、保护电流和地址 `23` 的写入。|
| [`feetech.py`](./src/lerobot/motors/feetech/feetech.py) | 这个文件定义飞特**舵机总线**，处理飞特**协议**和型号特有能力。 |SCS215 协议 `1` 声明不支持 **归中偏移** 和 **同步读取**，并跳过地址 `7` 的 **返回延迟时间**、地址 `41` 的 **加速度**写入。 |
| [`motors_bus.py`](./src/lerobot/motors/motors_bus.py) | 这个文件提供**通用串口电机**的读取、写入、同步写入和活动范围记录方法。 |根据电机适配器提供的**硬件能力属性**选择归中方式和读取方式。SCS215 由 `feetech.py` 声明不支持归中偏移和同步读取，因此校准返回零偏移，范围记录逐个读取位置。 |
| [`tables.py`](./src/lerobot/motors/feetech/tables.py) | 这个文件保存型号对应的**寄存器**地址、字节数和分辨率等固定数据。 |增加 **SCS215 型号映射**，记录对应的 **1024 级分辨率**、修正后的 **波特率表**、**负载方向编码**、**硬件型号编号** 和 **协议 1**。当代码传入型号名 `scs215` 时，连接检查会要求舵机返回硬件型号编号 `1315`。读取当前位置会访问地址 `56`，发送目标位置会访问地址 `42`，读取负载时解析第 `10` 位方向信息。避免访问地址 `7`、`23` 和 `41`。 |
