...
我们修改的四个文件，分别处理机械臂配置、飞特硬件型号配置、通用串口读写和硬件数值映射。
...
| --- | --- | --- |
| [`so_follower.py`](./src/lerobot/robots/so_follower/so_follower.py) |这个文件决定**机械臂配置**：机械臂有几个关节、每个关节使用什么舵机，并安排连接、校准、读取和运动。 |腕部旋转关节的最大原始位置改为从型号分辨率中读取。  六个关节的 **电机型号** 从 `sts3215` 改为 `scs215`，**总线协议** 改为 `1`，机身关节统一使用 **归一化位置**。| 
| [`motors_bus.py`](./src/lerobot/motors/motors_bus.py) | 这个文件提供**通用串口电机**的读取、写入、同步写入和活动范围记录方法。 |使用协议 `1` 时，校准过程把 **归中偏移** 记为 `0`，跳过 `scs215` 没有的归中偏移寄存器。记录 **关节活动范围** 时，通过 **逐个读取** 获得每个舵机的当前位置。 | 
| [`feetech.py`](./src/lerobot/motors/feetech/feetech.py) | 这个文件定义飞特**舵机总线**，处理飞特**协议**。 |电机型号是 `scs215` 时，跳过 `sts3215` 有而 `scs215` 没有的地址 `7` 的 **返回延迟时间**和地址 `41` 的 **加速度**写入。 | 
| [`tables.py`](./src/lerobot/motors/feetech/tables.py) | 这个文件保存型号对应的**寄存器**地址、字节数和分辨率等固定数据。 |增加 **SCS215 型号映射**，记录对应的 **1024 级分辨率**、**波特率表**、**编码方式**、**硬件型号编号** 和 **协议 1**。当代码传入型号名 `scs215` 时，连接检查会要求舵机返回硬件型号编号 `1315`。读取当前位置会访问地址 `56`，发送目标位置会访问地址 `42`。避免访问仅能用于原来 STS3215 的地址 `7`、`23` 和 `41` 对应的三个名称。 | 
